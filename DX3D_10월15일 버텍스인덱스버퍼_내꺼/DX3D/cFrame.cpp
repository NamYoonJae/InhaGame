...
void cFrame::Render()
{
	if (m_pMtlTex)
	{
		...
		g_pD3DDevice->DrawPrimitive(D3DPT_TRIANGLELIST, 0, m_nNumTri);//m_nNumTri 삼각형을 몇개를 그릴 것이냐로 넣는다
...
void cFrame::BuildVB(vector<ST_PNT_VERTEX>& vecVertex)
{
	...
	g_pD3DDevice->CreateVertexBuffer(vecVertex.size() * sizeof(ST_PNT_VERTEX), 0, ST_PNT_VERTEX::FVF, D3DPOOL_MANAGED, &m_pVB, NULL);
	ST_PNT_VERTEX * pV = NULL;
	m_pVB->Lock(0, 0, (LPVOID*)&pV, 0);	//락을 건다
	memcpy(pV, &vecVertex[0], vecVertex.size() * sizeof(ST_PNT_VERTEX));	//연속된 데이터라는 증거가 있어야 카피 가능
	m_pVB->Unlock();

}

void cFrame::BuildIB(vector<ST_ABC_INDEX>& vecABCIndex, vector<D3DXVECTOR3>& vecV)
{

	g_pD3DDevice->CreateVertexBuffer(vecV.size() * sizeof(D3DXVECTOR3), 0, , D3DPOOL_MANAGED, &m_pVB, NULL);


	g_pD3DDevice->CreateIndexBuffer(12 * sizeof(ST_ABC_INDEX), 0, D3DFMT_INDEX16, D3DPOOL_DEFAULT, &m_pIB, NULL);
	ST_ABC_INDEX *pABC = NULL;
	m_pIB->Lock(0, 0, (LPVOID*)&pABC, 0);
	memcpy(pABC, &vecABCIndex[0], vecABCIndex.size() * sizeof(ST_ABC_INDEX));
