...
{
	vector<D3DXVECTOR3> vecV;	//8개의 점
	vector<D3DXVECTOR2> vecVT;
	vector<ST_PNT_VERTEX> vecVertex;
	vector<ST_ABC_INDEX> vecABCIndex;	//ABC값
	...
	do
	{
		...
		{
			ProcessMESH_FACE_LIST(vecABCIndex, vecVertex, vecV);
		}
	...
	pFrame->SetIndex(vecABCIndex);
	pFrame->SetVecV(vecV);

	pFrame->BuildVB(vecVertex);
	pFrame->BuildIB(vecABCIndex, vecV);
...
void cAseLoader::ProcessMESH_FACE_LIST(OUT vector<ST_ABC_INDEX>&vecABCIndex, OUT vector<ST_PNT_VERTEX>& vecVertex, IN vector<D3DXVECTOR3>& vecV)
{
	int nLevel = 0;
	do
	{
