...
void cGrid::Setup(int nNumHarfTile, float fInterval)
{
	...
	for (float j = -15; j < 15; j++) 
	{
		...
		{
			v.c = D3DCOLOR_XRGB(128, 128, 128);
			v.p = D3DXVECTOR3(j+1, 0.0f, i);
			v.n = D3DXVECTOR3(0.0f, 1.0f, 0.0f);
			m_vecVertex.push_back(v);

			v.p = D3DXVECTOR3(j, 0.0f, i); 
			v.n = D3DXVECTOR3(0.0f, 1.0f, 0.0f);
			m_vecVertex.push_back(v);

			v.p = D3DXVECTOR3(j, 0.f, i + 1); 
			v.n = D3DXVECTOR3(0.f, 1.f, 0.f);
			m_vecVertex.push_back(v);

			v.p = D3DXVECTOR3(j+1, 0.f, i);
			v.n = D3DXVECTOR3(0.f, 1.f, 0.f);
			m_vecVertex.push_back(v);
