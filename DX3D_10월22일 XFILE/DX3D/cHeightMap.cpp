...
void cHeightMap::Setup(char * szFolder, char * szRaw, char * szTex, DWORD dwBytesPerPixel)
{
	...
	int nRow = (int)(sqrt((float)nNumVertex) + 0.0001f);	//오차여서 0.0001f를 더해줌
	int nCol = nRow;
	int nTileN = nRow - 1;
	m_nTileN = nTileN;
	...
	vecIndex.reserve(nTileN * nTileN * 2 * 3);	//resize 대신 reserve해도 된다
	...
	ZeroMemory(pA, (vecIndex.size() / 3) * sizeof(DWORD));	//0으로 써버릴 거니까 memcpy대신 ZeroMemory를 사용
